...
def selectionEmbed(user: str, opponent: str, userC: str, opponentC: str, round: int, embedColor):
    embed = discord.Embed(
    ...
    if opponentC != None:
        embed.add_field(name=f"{opponent}",
                        value=":white_check_mark: Selected")
    else:
...
def outcomeEmbed(user: str, opponent: str, userC: str, opponentC: str, round: int, userMonster: dict, opponentMonster: dict, outcome: tuple, userMult: dict, oppMult: dict, embedColor):
    totalUser = num_suffix(userMonster['Attack'] + userMonster['Defense'] +
                           userMonster['Intelligence'] + userMonster['Speed'])
    totalOpp = num_suffix(opponentMonster['Attack'] + opponentMonster['Defense'] +
                          opponentMonster['Intelligence'] + opponentMonster['Speed'])

    userStatLabels = {"attack": num_suffix(userMonster['Attack']), "defense": num_suffix(
        userMonster['Defense']), "intelligence": num_suffix(userMonster['Intelligence']), "speed": num_suffix(userMonster['Speed'])}
    oppStatLabels = {"attack": num_suffix(opponentMonster['Attack']), "defense": num_suffix(
        opponentMonster['Defense']), "intelligence": num_suffix(opponentMonster['Intelligence']), "speed": num_suffix(opponentMonster['Speed'])}
...
def finalEmbed(user: str, opponent: str, userMonster: dict, opponentMonster: dict, userMult: dict, oppMult: dict, totalUser: int, totalOpp: int, userChance: float, embedColor):
    ...
    embed.add_field(name="‎", value="‎", inline=True)
    embed.add_field(
        name=f"{opponent}:", value=f"{'{:.1f}'.format(100 - round(userChance * 100, 1))}% chance", inline=True)
...
def resultEmbed(user: str, opponent: str, userMonster: dict, opponentMonster: dict, duelType: str, userChance: float, winner: str, embedColor):
    totalUser = num_suffix(userMonster['Attack'] + userMonster['Defense'] +
                           userMonster['Intelligence'] + userMonster['Speed'])
    totalOpp = num_suffix(opponentMonster['Attack'] + opponentMonster['Defense'] +
                          opponentMonster['Intelligence'] + opponentMonster['Speed'])
    ...
    if winner == "user":
        embed = discord.Embed(
            title=f"{user} won against {opponent} with a {round(userChance * 100, 1)}% chance!", color=embedColor)
    else:
        embed = discord.Embed(
            title=f"{opponent} won against {user} with a {'{:.1f}'.format(100 - round(userChance * 100, 1))}% chance!", color=embedColor)
...
def duelOutcome(winner, duelType, userMonsterID, oppMonsterID, userID, oppID, origUserStats, origOppStats):
    if winner == "user":
        if duelType == "kill":
            interactor.edit_monster(userMonsterID, origUserStats['Attack'] + origOppStats['Attack'], origUserStats['Defense'] +
                                    origOppStats['Defense'], origUserStats['Speed'] + origOppStats['Speed'], origUserStats['Intelligence'] + origOppStats['Intelligence'])
            interactor.delete_monster(oppMonsterID)
            interactor.set_selected_monster(oppID, "None")
        elif duelType == "absorb":
            interactor.edit_monster(userMonsterID, origUserStats['Attack'] + origOppStats['Attack'] * 0.5, origUserStats['Defense'] + origOppStats['Defense']
                                    * 0.5, origUserStats['Speed'] + origOppStats['Speed'] * 0.5, origUserStats['Intelligence'] + origOppStats['Intelligence'] * 0.5)
            interactor.edit_monster(oppMonsterID, abs(origUserStats['Attack'] - origOppStats['Attack'] * 0.5), abs(origUserStats['Defense'] - origOppStats['Defense'] * 0.5), abs(
                origUserStats['Speed'] - origOppStats['Speed'] * 0.5), abs(origUserStats['Intelligence'] - origOppStats['Intelligence'] * 0.5))
        elif duelType == "steal":
        ...
        if duelType == "kill":
            interactor.edit_monster(oppMonsterID, origUserStats['Attack'] + origOppStats['Attack'], origUserStats['Defense'] +
                                    origOppStats['Defense'], origUserStats['Speed'] + origOppStats['Speed'], origUserStats['Intelligence'] + origOppStats['Intelligence'])
            interactor.delete_monster(userMonsterID)
            interactor.set_selected_monster(userID, "None")
        elif duelType == "absorb":
            interactor.edit_monster(oppMonsterID, origUserStats['Attack'] * 0.5 + origOppStats['Attack'], origUserStats['Defense'] * 0.5 +
                                    origOppStats['Defense'], origUserStats['Speed'] * 0.5 + origOppStats['Speed'], origUserStats['Intelligence'] * 0.5 + origOppStats['Intelligence'])
            interactor.edit_monster(userMonsterID, abs(origUserStats['Attack'] * 0.5 - origOppStats['Attack']), abs(origUserStats['Defense'] * 0.5 - origOppStats['Defense']), abs(
                origUserStats['Speed'] * 0.5 - origOppStats['Speed']), abs(origUserStats['Intelligence'] * 0.5 - origOppStats['Intelligence']))
        elif duelType == "steal":
...
class duelButtons(discord.ui.View):
    def __init__(self, *, timeout=180, user, opponent, userMonster, opponentMonster, duelType, duelColor):
        ...
        self.userMulti = {"attack": 0, "defense": 0,
                          "speed": 0, "intelligence": 0}
        self.opponentMulti = {"attack": 0,
    ...
    async def attack(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.edit_message(embed=selectionEmbed(self.user.display_name, self.opponent.display_name, self.userChoice, self.opponentChoice, self.round, embedColor=self.duelColor), view=self)
        ...
        if self.userChoice != None and self.opponentChoice != None:
            ...
            await interaction.edit_original_response(embed=outcomeEmbed(self.user.display_name, self.opponent.display_name, self.userChoice, self.opponentChoice, self.round, self.userMonster, self.opponentMonster, outcome=outcome, userMult=self.userMulti, oppMult=self.opponentMulti, embedColor=self.duelColor), view=self)
            ...
            if self.round != 3:
                self.round += 1
                await interaction.edit_original_response(embed=selectionEmbed(self.user.display_name, self.opponent.display_name, self.userChoice, self.opponentChoice, self.round, embedColor=self.duelColor), view=self)
            else:
                origUserStats = {"Attack": self.userMonster['Attack'], "Defense": self.userMonster['Defense'],
                                 "Intelligence": self.userMonster['Intelligence'], "Speed": self.userMonster['Speed']}
                origOppStats = {"Attack": self.opponentMonster['Attack'], "Defense": self.opponentMonster['Defense'],
                                "Intelligence": self.opponentMonster['Intelligence'], "Speed": self.opponentMonster['Speed']}

                for stat, decrease in self.userMulti.items():
                    self.userMonster[stat.capitalize(
                    )] *= (1 - (self.DECREASE_CONSTANT * decrease))
                for stat, decrease in self.opponentMulti.items():
                    self.opponentMonster[stat.capitalize(
                    )] *= (1 - (self.DECREASE_CONSTANT * decrease))

                totalUser = self.userMonster['Attack'] + self.userMonster['Defense'] + \
                    self.userMonster['Intelligence'] + \
                    self.userMonster['Speed']
                totalOpp = self.opponentMonster['Attack'] + self.opponentMonster['Defense'] + \
                ...
                await interaction.edit_original_response(embed=finalEmbed(user=self.user.display_name, opponent=self.opponent.display_name, userMonster=self.userMonster, opponentMonster=self.opponentMonster, userMult=self.userMulti, oppMult=self.opponentMulti, totalUser=totalUser, totalOpp=totalOpp, userChance=userChance, embedColor=self.duelColor))
                time.sleep(3)
                await interaction.edit_original_response(embed=resultEmbed(user=self.user.display_name, opponent=self.opponent.display_name, userMonster=self.userMonster, opponentMonster=self.opponentMonster, duelType=self.duelType, userChance=userChance, winner=winner, embedColor=self.duelColor))

                duelOutcome(winner, self.duelType, self.userMonster['ID'], self.opponentMonster['ID'], self.user.id, self.opponent.id, origUserStats, origOppStats)
...
class Dueling(commands.Cog):
    ...
    async def duel(self, ctx, offer, opponent: discord.Member = None):
        if not interactor.does_user_exist(ctx.author.id):
            ...
            await ctx.send("You or your opponent are already in a duel! Wait for the ongoing duel to finish.")
        else:
            ...
            challenge = discord.Embed(title=f"{ctx.author.display_name} has challenged you to a **{offer}** duel!",
                                      description="Will you accept? Reply with `duel accept` or `duel deny`", color=shade)
            challenge.add_field(name=f"{ctx.author.display_name}'s Nebby:",
            ...
            challenge.add_field(name=f"{opponent.display_name}'s Nebby:",
            ...
            if msg.content == "duel deny":
                ...
                embed = discord.Embed(
                    title=f"{ctx.author.display_name} VS {opponent.display_name} - Round 1/3")
                embed.add_field(
                    name=f"{ctx.author.display_name}", value=":x: Not Selected")
                embed.add_field(
                    name=f"{opponent.display_name}", value=":x: Not Selected")
                embed.add_field(name="‎", value="‎", inline=True)
                await ctx.send(embed=embed, view=duelButtons(user=ctx.author, opponent=opponent, userMonster=userMonster, opponentMonster=opponentMonster, duelType=offer, duelColor=shade))
